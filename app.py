...
st.set_page_config(
    page_title="AI Risk Report",
    layout="wide"
)

st.title("AI Risk Report")
st.write(
    "Upload a CSV file and generate an executive AI Risk Report showing dirty data exposure, "
    "segmentation risk, and operational AI readiness."
)
...
def calculate_segmentation_risk(df, likely_cols):
    ...
    for col in likely_cols["category"]:
...
def generate_report(df, file_name):
    ...
    report.append("")
    report.append(f"File analysed: {file_name}")
    ...
    return "\n".join(report), exposure_score, band, metrics, segmentation_score, segmentation_risks
...
if uploaded_file is not None:
    ...
    report, exposure_score, band, metrics, segmentation_score, segmentation_risks = generate_report(
        df,
        uploaded_file.name
    )

    st.success("CSV uploaded and analysed successfully.")

    col1, col2, col3 = st.columns(3)
    col1.metric("Dirty Data Exposure Score", f"{exposure_score}/100")
    col2.metric("Risk Band", band)
    col3.metric("Segmentation Risk Score", f"{segmentation_score}/100")

    st.subheader("Executive Summary")
    st.write(
        f"The uploaded file shows a Dirty Data Exposure Score of **{exposure_score}/100**, "
        f"placing it in the **{band}** AI risk band."
    )
    ...
    st.write(f"Rows analysed: {metrics['total_rows']}")
    st.write(f"Columns analysed: {metrics['total_cols']}")
    st.write(f"Missing cells: {metrics['missing_cells']} ({metrics['missing_pct']}%)")
    st.write(f"Duplicate rows: {metrics['duplicate_rows']} ({metrics['duplicate_pct']}%)")
    st.write(f"Blank/Unknown/Misc values: {metrics['blank_or_unknown_count']} ({metrics['blank_or_unknown_pct']}%)")

    st.subheader("Segmentation Risk Indicators")
    if segmentation_risks:
        for risk in segmentation_risks:
            st.warning(risk)
    else:
        st.success("No major visible segmentation risks detected.")

    st.subheader("Full AI Risk Report")
    st.text_area("Report", report, height=500)

    st.download_button(
        label="Download AI Risk Report TXT",
        data=report,
        file_name="AI_Risk_Report.txt",
        mime="text/plain"
    )
